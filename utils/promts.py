def get_system_prompt(section: str, knowledge: str) -> str:
    ...
    11. Отвечай красиво оформив текст, чтобы было удобно читать и визуально был выделен акцент
    12. Обязательно вставляй по смыслу ссылки на статью или статьи, которые
    по смыслу относятся к впросу. Ссылки оформи как гиперссылку
    13. Если не до конца ясно что хотел пользователь сообщи это и попроси уточнить что именно он имеет ввиду.

    тобы выделить текст жирным используй вместо * звездочки html теги <b>
Вставляй ссылки как гиперссылки, учитывай что это телеграм, сипользуй HTML разметку
например:
    <a href="https://wiki.hr-link.ru/...">Добавить отдел</a>
    Не используй Markdown-разметку для ссылок ([текст](ссылка)).
...
STARTLINK - https://wiki.hr-link.ru/bin/view/Start%20Link/
"""


hr_link_docs = {
    "Переключение в режим 'Сотрудник'": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Lichnyi-kabinet-/Perekliuchenie-v-rezhim-«Sotrudnik»-/",
    "Загрузить документ": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Dokumenty-/Zagruzitь-dokument/",
    "Настроить маршрут согласования документа": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Dokumenty-/Nastroitь-marshrut-soglasovaniia-dokumenta-/",
    "Добавить в документ более одного сотрудника": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Dokumenty-/Dobavitь-v-dokument-bolee-odnogo-sotrudnika-/",
    "Отправить документ на подпись": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Dokumenty-/Otpravitь-dokument-na-podpisь/",
    "Удалить или аннулировать документ": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Dokumenty-/Udalitь-ili-annulirovatь-dokument-/",
    "Скачать архив документооборота": "https://wiki.hr-link.ru/bin/view/Main/Kadrovik-/Dokumenty-/Skachatь-arkhiv-dokumentooborota-/",
    ...
    "Назначить роль: Руководитель, Кадровик, Делопроизводитель": "https://wiki.hr-link.ru/bin/view/Main/Administrator-/Sotrudniki-/Naznachitь-rolь%3A-Rukovoditelь%2C-Kadrovik%2C-Deloproizvoditelь-/",
    "Администратор, Назанчить роль: кадровик, Руководитель, Делопроизводитель; Наделить пользователя правами администратора и настройщика; управлять правами кадровиков и руководителей": "https://wiki.hr-link.ru/bin/view/Main/Administrator-/Sotrudniki-/",
    "Администратор, справочники, МЧД Создать МЧД вручную Создать МЧД из файла Копировать, редактировать и отправлять на подпись черновика МЧД Карточка МЧД: скачать, удалить, восстановить удалённую доверенность; Типы заявлений Создать новый тип заявления или редактировать существующий": "https://wiki.hr-link.ru/bin/view/Main/Administrator-/Spravochniki-/",
    "STARTLINK": "https://wiki.hr-link.ru/bin/view/Start%20Link/",
    ...
    "Принять приглашение и заполнить шаблон запроса документов": "https://wiki.hr-link.ru/bin/view/Start%20Link/Kandidat-/Priniatь-priglashenie-i-zapolnitь-"
}
